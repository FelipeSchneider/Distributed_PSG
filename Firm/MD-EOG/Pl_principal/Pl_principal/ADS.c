...
{
	PORTH_DIRCLR = ALARM_ADS;				//ALARM pins as input
	
	PORTF_DIRSET = SS_ADS|SCK_ADS|MOSI_ADS;			//as output: MOSI, SCK, SS
	PORTF_OUTSET = SS_ADS;							//Slave is deactivate
	PORTF_DIRCLR = MISO_ADS | DRDYB_ADS;						//MISO and DRDYB as input
...
// 	PORTF_DIRCLR = DRDYB_ADS;
// 	PORTF_PIN3CTRL = PORT_OPC_PULLUP_gc | 1;		//activates the pull ups
// 	
// 	PORTF_INTCTRL = 2;	//Level baixo para int0 do portF
// 	PORTF_INT0MASK = DRDYB_ADS;	//Pino 3 será fonte de interrupção
// 	//PORTF_PIN3CTRL |= 1;			//RISING edge
// 	PORTF_INTFLAGS = 1;//clear the interrupt flag

	PORTF_INTCTRL = 1;	//Level baixo para int0 do portE
	PORTF_INT0MASK = PIN3_bm;	//Pino 3 será fonte de interrupção
	PORTF_PIN3CTRL = 0b00010001;			//falling edge
	PORTF_INTFLAGS = 1;//clear the interrupt flag
}