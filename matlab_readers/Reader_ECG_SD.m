...
fprintf('Esta placa armazenou %d segundos',segundos);
...
while(cont_exit<=length(rec_message))
    ...
    while (f_headers_ok == 0) %procurando pelos headers
        ...
        if (cont_problem > (3+jump_problem))
           fprintf('Segundo %d NÃO está OK, problema com os headers',round(cont_exit/one_second_MD_EOG));
...
filt_acc_x_ecg = [Dump;g_acc_x]; filt_acc_x_ecg = filter(b,a,filt_acc_x_ecg);
filt_acc_x_ecg = filt_acc_x_ecg(50001:end);

Dump = ones(50000,1); Dump = Dump.*g_acc_y(1);
filt_acc_y_ecg = [Dump;g_acc_y]; filt_acc_y_ecg = filter(b,a,filt_acc_y_ecg);
filt_acc_y_ecg = filt_acc_y_ecg(50001:end);

Dump = ones(50000,1); Dump = Dump.*g_acc_z(1);
filt_acc_z_ecg = [Dump;g_acc_z]; filt_acc_z_ecg = filter(b,a,filt_acc_z_ecg);
filt_acc_z_ecg = filt_acc_z_ecg(50001:end);
...
subplot(2,2,2); plot(acc_time,filt_acc_x_ecg);ylabel('Eixo X [m/s²]'); grid on;xlabel('Tempo [s]');axis([0 segundos -inf inf]) 
subplot(2,2,3); plot(acc_time,filt_acc_y_ecg);ylabel('Eixo Y [m/s²]');grid on; axis([0 segundos -inf inf])
subplot(2,2,4); plot(acc_time,filt_acc_z_ecg);ylabel('Eixo Z [m/s²]');grid on; xlabel('Tempo [s]');axis([0 segundos -inf inf])
suptitle(sprintf('MD-ECG Paciente%d, Hora %02d',paciente,hora));

run('EDR_extraction.m');
