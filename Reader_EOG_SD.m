...
if(plot_all)
    ...
    figure; 
    subplot(3,1,1);plot(acc_time,filt_acc_x);ylabel('Eixo X [m/s²]'); grid on;
    subplot(3,1,2);plot(acc_time,filt_acc_y);ylabel('Eixo Y [m/s²]'); grid on;
    subplot(3,1,3);plot(acc_time,filt_acc_z);ylabel('Eixo Z [m/s²]'); grid on; xlabel('Tempo [s]');
    figure; 
...
subplot(4,2,7); plot(temp_time,rec_temp_hdc);ylabel('Temp. [°C]');grid on; axis([0 segundos -inf inf]); xlabel('Tempo [s]');
subplot(4,2,2); plot(temp_time,rec_umd_hdc);ylabel('Umidade[%]');grid on; axis([0 segundos -inf inf])
subplot(4,2,4); plot(acc_time,filt_acc_x);ylabel('Acc. X [m/s²]'); grid on; axis([0 segundos -inf inf])
subplot(4,2,6); plot(acc_time,filt_acc_y);ylabel('Acc. Y [m/s²]'); grid on; axis([0 segundos -inf inf])
